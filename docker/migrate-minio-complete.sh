...
# MinIO 数据完整迁移脚本
# 
# 功能：将 MinIO 数据（对象存储）从满载的根分区迁移到 /mnt/data6t
...
NEW_DATA_DIR="/mnt/data6t/ragflow_minio"
VOLUME_NAME="docker_minio_data"
...
if [ -d "$OLD_DATA_PATH" ]; then
    ...
    DATA_SIZE=$(sudo du -sh "$OLD_DATA_PATH" 2>/dev/null | awk '{print $1}')
    echo "  大小: $DATA_SIZE"
    
    # 显示 MinIO bucket 信息
    if [ -d "$OLD_DATA_PATH" ]; then
        echo "  Buckets:"
        sudo ls -lh "$OLD_DATA_PATH" 2>/dev/null | grep "^d" | awk '{print "    - " $NF}' || echo "    无数据"
    fi
else
...
# 确认是否继续
read -p "确认开始迁移 MinIO 数据? (y/n): " confirm
if [ "$confirm" != "y" ]; then
    echo "操作已取消"
    exit 0
fi
...
# ============================================================================
echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${YELLOW}步骤 4/8: 迁移 MinIO 数据${NC}"
echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
if [ -d "$OLD_DATA_PATH" ] && [ "$(sudo ls -A $OLD_DATA_PATH 2>/dev/null)" ]; then
    ...
    echo ""
    echo "开始迁移数据..."
    echo "提示：MinIO 包含文档、图片等文件，迁移时间取决于数据量"
    echo ""
    ...
    # 显示迁移的 bucket
    echo "迁移的 Buckets:"
    sudo ls -lh "$NEW_DATA_DIR" 2>/dev/null | grep "^d" | awk '{print "  - " $NF}' || echo "  无数据"
else
    echo -e "${YELLOW}⚠️  未找到现有数据，将创建空数据目录${NC}"
    echo "   MinIO 启动后会自动创建必要的目录结构"
fi
echo ""
...
# 备份当前配置文件
BACKUP_FILE="${COMPOSE_FILE}.minio_migration_backup_$(date +%Y%m%d_%H%M%S)"
...
# 跳过旧的 minio_data 的 driver: local 行
skip_old_minio == 1 && /^    driver: local/ {
    skip_old_minio = 0
    next
}
...
echo ""
echo "minio_data 配置："
grep -A 6 "^  minio_data:" "$COMPOSE_FILE" || echo "未找到 minio_data 定义"
echo ""
...
echo "1. 容器状态："
docker ps --format "table {{.Names}}\t{{.Status}}" | grep -E "NAME|minio" || true
echo ""

echo "2. MinIO 健康检查："
docker exec ragflow-minio curl -sf http://localhost:9000/minio/health/live 2>/dev/null && \
    echo "✓ MinIO 健康状态正常" || \
    echo "等待 MinIO 完全启动..."
echo ""

echo "3. MinIO 数据目录挂载："
docker inspect ragflow-minio | grep -A 3 "Source.*ragflow_minio" 2>/dev/null || \
    docker inspect ragflow-minio | grep -A 5 '"Mounts"' | grep -A 3 "/data"
echo ""
...
echo -e "${BLUE}🎯 验证步骤：${NC}"
echo "  1. 访问 MinIO 控制台: http://localhost:9002"
echo "     用户名: $MINIO_USER"
echo "     密码: $MINIO_PASSWORD"
echo "  2. 检查数据目录: ls -lh $NEW_DATA_DIR"
echo "  3. 测试 RAGFlow 文档上传功能"
echo ""

echo -e "${BLUE}💡 提示：${NC}"
echo "  MinIO 存储了所有上传的文档和生成的文件"
echo "  确保迁移后能正常访问历史文档"
echo ""
